...
function getUser(PDO $db, string $userId): ?array
{
...
function updateUser(PDO $db, array $userData): void
{
    $stmt = $db->prepare("
...
function addMessage(PDO $db, array $messageData): int
{
    $stmt = $db->prepare("
...
function approveMessage(PDO $db, int $messageId): void
{
    $stmt = $db->prepare("
        UPDATE messages
...
function banUser(PDO $db, string $userId): void
{
    $stmt = $db->prepare("
        UPDATE users
        SET is_banned = 1
...
function unbanUser(PDO $db, string $userId): void
{
    $stmt = $db->prepare("
        UPDATE users
        SET is_banned = 0
